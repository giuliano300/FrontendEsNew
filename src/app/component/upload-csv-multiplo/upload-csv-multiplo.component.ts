...
export class UploadCsvMultiploComponent {
  ...
  page: number = TourPage.uploadCsvMulti;
        ...
        if(!step1)
          ...
          switch(parseInt(datiDecriptati.prodotto)){
            ...
            case ProductTypes.MOL:
              this.backLink = "/invioMultiploRaccomandata2";
  ...
  parseCsv(csvText: string): { recipient: Recipients, bulletin?: Bulletins }[] {
    ...
    startTour() {
      const steps = [
        {
          id: 'uploadmulti',
          text: 'Carcia una lista di destinatari in formato .CSV <br><em>(sotto puoi scaricare il file di esempio)</em>',
          attachTo: {
            element: '.step-1',
